...
app.get('/api/setup', async (req, res) => {
    try {
        ...
        if (existing) return res.json({ message: 'Admin already exists. Login with admin@jobijad.com / admin123' });
        await User.create({ name: 'Admin', email: 'admin@jobijad.com', password: 'admin123', role: 'admin' });
        res.json({ message: 'Done! Login with admin@jobijad.com / admin123' });
...
// [seed-products route removed]
app.get('/api/seed-products', async (req, res) => {
    try {
        const Product = require('./models/Product');
        const count = await Product.count();
        if (count > 0) return res.json({ message: count + ' products already exist. Use admin panel to manage products.' });
        const products = [
    ...
    .then(async () => {
        ...
        try {
            ...
            if (!adminExists) {
                ...
                console.log('Auto-seeded admin & demo user.');
            }
        } catch(e) { console.error('User seed failed:', e.message); }
        ...
        } catch(e) { console.error('Product seed failed:', e.message); }
